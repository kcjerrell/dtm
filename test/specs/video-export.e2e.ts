...
function findFfprobePath(): string {
	const entries = fs.existsSync(ffmpegBinDir)
		? fs.readdirSync(ffmpegBinDir)
		: [];
	...
	const fallback = ["/opt/homebrew/bin/ffprobe", "/usr/local/bin/ffprobe"].find(
		(p) => fs.existsSync(p),
	);
...
describe("Video Export", () => {
	it("can export a video", async () => {
		...
		const showVideosPressed =
			await showVideosToggle.getAttribute("aria-pressed");
		...
		await expect(
			$('[aria-label="Mute video"], [aria-label="Unmute video"]'),
		).not.toExist();
		...
		await browser.execute((folderPath) => {
			...
			(window as E2EWindow).__E2E_FILE_PATH__ = folderPath;
		}, videoOutputPath);
