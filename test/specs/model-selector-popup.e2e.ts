...
describe("Model Selector Popup", () => {
    ...
    it("keeps model selector open for related form interactions", async () => {
        ...
        const versionItem = $('[aria-label*="Model version FLUX.1"]')
        await versionItem.waitForDisplayed({ timeout: 15000 })
        await versionItem.click()

        // Model item FLUX.1 [dev] (5-bit)
        const modelA = $('[aria-label*="Model item FLUX.1 [dev] (5-bit)"]')
        await modelA.click()
        await expectModelListOpen()

        const modelB = $('[aria-label*="Chroma1 HD (5-bit)"]')
        await modelB.click()
        await expectModelListOpen()
        ...
        const removeButtons = await $$('[aria-label^="Remove selected model "]').getElements()
