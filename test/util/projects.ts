...
export async function removeProject(folder: TestFolder, project: TestProject | string) {
    ...
    const files = [`${base}.sqlite3`, `${base}.sqlite3-shm`, `${base}.sqlite3-wal`]
...
export function mountDmg(dmgPath: string) {
    const output = execSync(`hdiutil attach "${dmgPath}" -nobrowse -plist`, {
        encoding: "utf8",
    })

    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    // biome-ignore lint/suspicious/noExplicitAny: hdiutil returns non-standard plist
    const parsed = plist.parse(output) as any

    // Find the mount point
    // biome-ignore lint/suspicious/noExplicitAny: ...
    const mountPoint = parsed["system-entities"].map((e: any) => e["mount-point"]).find(Boolean)
