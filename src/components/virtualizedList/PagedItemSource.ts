...
export class PagedItemSource<T> implements IItemSource<T> {
    ...
    findItem(predicate: (item: T) => boolean): T | undefined {
        ...
        // search the preceding range
        for (const item of getRenderItems(0, this.state.firstIndex - 1, this.pages)) {
            if (item && predicate(item)) return item
        }
        // search the succeeding range
        for (const item of getRenderItems(
        ...
        await this.pageLoader.runExclusive(async () => {
            ...
            if (clearOthers) {
                this.pages = this.pages.filter(
                    (p) => p.from >= firstPage * this.pageSize && p.to <= lastPage * this.pageSize,
                )
            }
...
export class EmptyItemSource<T> implements IItemSource<T> {
    ...
    findItem(_: (item: T) => boolean): T | undefined {
        return undefined
    }
