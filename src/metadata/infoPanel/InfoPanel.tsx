            ...
            >
                <Tabs.List>
                    {["details", "config"].map((tab) => (
                        <Tabs.Trigger aria-label={`${capitalize(tab)} tab`} key={tab} value={tab}>
                            {capitalize(tab)}
                        </Tabs.Trigger>
                    ))}
                    <Spacer />
                ...
                >
                    {currentImage && (
                        <Details
                            ...
                            expandItems={Object.keys(
                                expandedSnap[currentImage.id]?.expanded ?? [],
                            ).map((k) => k.replace("details_", ""))}
                            onItemCollapseChanged={(subkey, collapse) => {
                                if (!uiState.current?.[currentImage.id]) return
                                const key = `details_${subkey}`
                                if (collapse === "collapsed")
                                    delete uiState.current[currentImage.id].expanded[key]
