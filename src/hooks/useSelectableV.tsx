...
function clearAllSelected<T extends Selectable>(
...
function selectItem<T extends Selectable>(
  ...
  // multiple modifier mode
    ...
    if (modifier === "cmd") {
      ...
      const start = itemsSnap.findIndex(
        (it) => state.keyFn(it) === state.keyFn(lastItem),
      );
      const end = itemsSnap.findIndex(
        (it) => state.keyFn(it) === state.keyFn(item),
      );
...
export function useSelectableGroup<T extends Selectable>(
  ...
  const { mode, keyFn, onSelectionChanged } = {
    ...defaultSelectableGroupOptions,
    ...opts,
  };
  ...
  if (
    onSelectionChanged &&
    onSelectionChanged !== onSelectionChangedRef.current
  ) {
...
export function useSelectable<T extends Selectable>(item: T) {
  ...
  if (!context)
    throw new Error("useSelectable must be used within a SelectableGroup");
...
export function makeSelectable<T extends object>(
  ...
  const onClick =
    handleClick ??
    ((item: Selectable<T>, value: boolean) => item.toggleSelected());
  ...
  const p = proxy({
    ...
    onClick(e?: React.MouseEvent) {
      const modifier =
        e && hasPrimaryModifier(e) ? "cmd" : e?.shiftKey ? "shift" : undefined;
...
export function makeSelectableList<T extends object>(
  items: T[],
): Selectable<T>[] {
