...
export class Container<T extends object> {
	...
	private static constructorStack: Container<object>[] = []

	static register<T extends object>(name: keyof T, service: T[typeof name]) {
		const container = Container.constructorStack.at(-1) as Container<T>
