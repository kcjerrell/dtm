...
function ModelValueSelectorComponent(
    ...
    const { state, snap } = useProxyRef(() => ({
        ...
        versions: [] as [
            string,
            { models: number; loras: number; controls: number; label: string },
        ][],
    ...
    useEffect(() => {
        return () => {
            uiState.setGridInert(false)
        }
    }, [uiState])

    const showList = (value: boolean) => {
        state.isOpen = value
        uiState.setGridInert(value)
    }

    const contentPane = useRootElement("viewContent")
