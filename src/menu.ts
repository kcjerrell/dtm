...
async function createAppMenus() {
  ...
  const clearHistoryUnsub = subscribeSetting(
    "metadata.clearHistoryOnExit",
    async () => {
      await updateMenu();
    },
  );
  const clearPinsUnsub = subscribeSetting(
    "metadata.clearPinsOnExit",
    async () => {
      await updateMenu();
    },
  );
