...
pub fn get_clipboard(pasteboard: Option<String>) -> anyhow::Result<Retained<NSPasteboard>> {
    unsafe {
        match pasteboard.as_deref() {
            ...
            Some(other) => anyhow::bail!("Unknown pasteboard name: {other}"),
        }
    }
}

pub fn write_clipboard_binary(ty: String, data: Vec<u8>) -> anyhow::Result<()> {
    ...
    if !ok {
        anyhow::bail!("Failed to write binary data for {}", ty);
    }

    Ok(())
}

pub fn read_clipboard_binary(ty: String, pasteboard: Option<String>) -> anyhow::Result<Vec<u8>> {
    let pb = get_clipboard(pasteboard)?;
    let ns_type = NSString::from_str(&ty);
    let type_array = NSArray::from_slice(&[&*ns_type]);

    if pb.availableTypeFromArray(&*type_array).is_none() {
        anyhow::bail!("Type {} not available", ty);
    }

    let data: Option<Retained<NSData>> = pb.dataForType(&*ns_type);
    let data = data.ok_or_else(|| anyhow::anyhow!("Failed to read binary data for {}", ty))?;
...
pub fn read_clipboard_types(pasteboard: Option<String>) -> anyhow::Result<Vec<String>> {
    ...
    let available = pb.types().ok_or_else(|| anyhow::anyhow!("Failed to get available types"))?;
